        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-red-sunglo bold uppercase">Add Subcategory</span>
                </div>
            </div>
            <div class="portlet-body form">
                <form method="post" action="{{route('admin-subcat-store')}}" class="form-horizontal" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="form-body">
                            ...
                            <div class="col-md-10">
                                <select  name="category_id" required="" class="form-control">
                                  <option value="">{{ __("Select Category") }}</option>
                                    @foreach($cats as $cat)
                                      <option value="{{ $cat->id }}" {{ (old('category_id')!="") ? ($cat->id==old('category_id'))?'selected':'' : ''}}>{{ $cat->name }}</option>
                                    @endforeach
                                </select>
